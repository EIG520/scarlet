...
use bullet::game::formats::viriformat::{
    self,
    chess::{board::GameOutcome, chessmove::MoveFlags, piece::PieceType},
};
...
impl DataGenner {
    ...
    pub async fn gen_game(&mut self, randmoves: usize, softnodes: u128) {
        ...
        let mut sgame =
            viriformat::dataformat::Game::new(&self.virigame.initial_position.unpack().0);
    ...
    pub fn play_scarlet_move(&mut self, softnodes: u128) {
        ...
        self.virigame
            .add_move(scarmv_to_virimv(mv), searcher.white_eval() as i16);
        self.viriboard.make_move_simple(scarmv_to_virimv(mv));
        self.scarboard.make_move(&mv);
...
pub fn scarmv_to_virimv(mv: Move) -> viriformat::chess::chessmove::Move {
    ...
    let fromsq =
        unsafe { types::Square::default().add_unchecked(mv.from.trailing_zeros() as u8 ^ 7) };
    ...
    match mv.flag {
        ...
        Flag::WhiteEnPassant => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, tosq, MoveFlags::EnPassant)
        }
        Flag::BlackEnPassant => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, tosq, MoveFlags::EnPassant)
        }
        ...
        Flag::KnightPromotion => {
            viriformat::chess::chessmove::Move::new_with_promo(fromsq, tosq, PieceType::Knight)
        }
        Flag::BishopPromotion => {
            viriformat::chess::chessmove::Move::new_with_promo(fromsq, tosq, PieceType::Bishop)
        }
        Flag::RookPromotion => {
            viriformat::chess::chessmove::Move::new_with_promo(fromsq, tosq, PieceType::Rook)
        }
        Flag::QueenPromotion => {
            viriformat::chess::chessmove::Move::new_with_promo(fromsq, tosq, PieceType::Queen)
        }

        Flag::WhiteKingsideCastle => viriformat::chess::chessmove::Move::new_with_flags(
            fromsq,
            types::Square::H1,
            MoveFlags::Castle,
        ),
        Flag::BlackKingsideCastle => viriformat::chess::chessmove::Move::new_with_flags(
            fromsq,
            types::Square::H8,
            MoveFlags::Castle,
        ),
        Flag::WhiteQueensideCastle => viriformat::chess::chessmove::Move::new_with_flags(
            fromsq,
            types::Square::A1,
            MoveFlags::Castle,
        ),
        Flag::BlackQueensideCastle => viriformat::chess::chessmove::Move::new_with_flags(
            fromsq,
            types::Square::A8,
            MoveFlags::Castle,
        ),
