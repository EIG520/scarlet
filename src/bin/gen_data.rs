...
async fn main() {
    ...
    let mut file =
        BufWriter::new(OpenOptions::new().create(true).append(true).open("train7.viri").expect("couldn't open file"));
...
impl DataGenner {
    ...
    pub fn play_scarlet_move(&mut self, softnodes: u128) {
        ...
        let mut searcher =
            Searcher::new(&mut self.scarboard, &mut tt, StoredOptions { use_tt: true, t: TuneBox::default() });
...
pub fn scarmv_to_virimv(mv: Move) -> viriformat::chess::chessmove::Move {
    ...
    match mv.flag {
        ...
        Flag::WhiteKingsideCastle => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, types::Square::H1, MoveFlags::Castle)
        }
        Flag::BlackKingsideCastle => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, types::Square::H8, MoveFlags::Castle)
        }
        Flag::WhiteQueensideCastle => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, types::Square::A1, MoveFlags::Castle)
        }
        Flag::BlackQueensideCastle => {
            viriformat::chess::chessmove::Move::new_with_flags(fromsq, types::Square::A8, MoveFlags::Castle)
        }
    }
}
